...
- **从“传统页面”到 Vue3 SPA 的重构实践**
  - 完整对照老版 `app.js` 中的大量内联样式与 DOM 结构，将其拆分为多个 Vue3 组件，保留原始 UI/交互习惯，同时去掉 Element Plus，改为**纯原生标签 + 内联样式**实现。
  - 通过 Pinia 管理全局用户信息，实现**不用重新登录即可实时刷新状态**（员工休假 / 在职变更、资料修改等）。
...
- **原生 HTML + 内联 CSS**
  - 原本项目大量样式写在老版 `app.js` 中，改造时**没有继续依赖 UI 组件库**，而是：
    - 在每个 Vue 组件中使用原生标签（`div / table / button / input / select` 等）+ `style` 属性，还原旧系统的布局和配色。
    - 手写了卡片、表格、按钮、弹窗、表单布局等样式，保证“看上去还是原来的系统，但代码是组件化的”。
